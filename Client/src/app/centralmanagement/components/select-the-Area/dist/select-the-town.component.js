...
var SelectTheAreaComponent = /** @class */ (function () {
    ...
    SelectTheAreaComponent.prototype.addItem = function () {
        ...
        this.fval.selectAreas.push(this.selectArea);
    };
    SelectTheAreaComponent.prototype.removeItem = function (index) {
        this.fval.selectAreas.removeAt(index);
    };
    SelectTheAreaComponent.prototype.initialiseForm = function () {
        ...
        this.approvedAreas.forEach(function (item, i) {
            ...
            // console.log(i);
            _this.fval.selectArea.controls[i].controls.aeaId.setValue(item.areaId);
            _this.fval.selectArea.controls[i].controls.areaName.setValue(item.areaName);
            _this.fval.selectArea.controls[i].controls.approved.setValue(false);
            _this.addItem();
            n = i + 1;
        });
        this.removeItem(n);
    ...
    SelectTheAreaComponent.prototype.checkAllItems = function (val) {
        ...
        if (val === true) {
            this.approvedAreas.forEach(function (item, i) {
                _this.fval.selectAreas.controls[i].controls.approved.setValue(val);
            });
        }
        else {
            this.approvedAreas.forEach(function (item, i) {
                _this.fval.selectAreas.controls[i].controls.approved.setValue(false);
            });
        }
    };
    SelectTheAreaComponent.prototype.deselectAll = function (val) {
        // console.log(this.fval.approveAreas["controls"][val]["controls"].approved.value)
        if (this.fval.selectAreas.controls[val].controls.approved.value === true) {
            this.fval.selectAll.setValue(false);
        }
    };
    ...
    SelectTheAreaComponent = __decorate([
        core_1.Component({
            selector: 'app-lib-select-the-town',
            templateUrl: './select-the-town.component.html',
            styleUrls: ['./select-the-town.component.scss']
        })
