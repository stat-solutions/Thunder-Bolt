...
export class ReverseInterestComponent implements OnInit {
    ...
    this.others.getReversedInterestsForApproval().subscribe(
      items => {
        ...
        this.txnsApprovals.forEach((item, i) => {
          ...
          this.fval.txnApprovals.controls[i].controls.amount.setValue(details.theInterestToBeReversed);
    ...
    this.txnsApprovals.forEach((item, i) => {
      if (this.fval.txnApprovals.controls[i].controls.approved.value === true) {
        itemsApproved.push({
          ...
          theInterestToBeReversed: this.fval.txnApprovals.controls[i].controls.amount.value
        });
      }
    });
    // console.log(itemsApproved);
    if (itemsApproved.length > 0) {
      this.others.postApproveReverseInterest(itemsApproved).subscribe(
        res => {
          this.posted = true;
          this.alertService.success({
  ...
  rejectItems(): any {
    ...
    if (itemsRejected.length > 0) {
      this.others.postRejectReverseInterest(itemsRejected).subscribe(
        res => {
          this.posted = true;
          this.alertService.success({
