...
export class LoanLimitComponent implements OnInit {
    ...
    this.others.getIdividualLoanLimit().subscribe(
      items => {
        ...
        this.txnsApprovals.forEach((item, i) => {
          const details = JSON.parse(item.approvalLoanLimitPayLoad);
    ...
    this.txnsApprovals.forEach((item, i) => {
      if (this.fval.txnApprovals.controls[i].controls.approved.value === true) {
        itemsApproved.push({
          ...
          approvalLoanLimitId: this.fval.txnApprovals.controls[i].controls.approvalLoanLimitId.value,
          theLoanLimit: this.fval.txnApprovals.controls[i].controls.limit.value
        });
      }
    });
    // console.log(itemsApproved);
    if (itemsApproved.length > 0) {
      this.others.approveIdividualLoanLimit(itemsApproved).subscribe(
        res => {
          this.posted = true;
          this.alertService.success({
  ...
  rejectItems(): any {
    ...
    if (itemsRejected.length > 0) {
      this.others.rejectIdividualLoanLimit(itemsRejected).subscribe(
        res => {
          this.posted = true;
          this.alertService.success({
