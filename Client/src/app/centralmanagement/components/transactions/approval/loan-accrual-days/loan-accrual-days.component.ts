    ...
    this.others.getIdividualLoanAccrualDays().subscribe(
      items => {
        ...
        this.txnsApprovals.forEach((item, i) => {
          ...
          this.fval.txnApprovals.controls[i].controls.days.setValue(details.theLoanAccrualDays);
    ...
    this.txnsApprovals.forEach((item, i) => {
      if (this.fval.txnApprovals.controls[i].controls.approved.value === true) {
        itemsApproved.push({
          ...
          theLoanAccrualDays: this.fval.txnApprovals.controls[i].controls.days.value
        });
      }
    });
    // console.log(itemsApproved);
    if (itemsApproved.length > 0) {
      this.others.approveIdividualLoanAccrualDays(itemsApproved).subscribe(
        res => {
          this.posted = true;
          this.alertService.success({
