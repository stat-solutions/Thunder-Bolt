...
export class ReversePrincipleComponent implements OnInit {
    ...
    this.others.getReversedPrincipalForApproval().subscribe(
      items => {
        ...
        this.txnsApprovals.forEach((item, i) => {
          ...
          this.fval.txnApprovals.controls[i].controls.amount.setValue(details.theLoanInterestamount);
    ...
    this.txnsApprovals.forEach((item, i) => {
      if (this.fval.txnApprovals.controls[i].controls.approved.value === true) {
        itemsApproved.push({
          ...
          thePrincipalToBeReversed: this.fval.txnApprovals.controls[i].controls.amount.value
        });
      }
    });
    // console.log(itemsApproved);
    if (itemsApproved.length > 0) {
      this.others.postApproveReversePrincipal(itemsApproved).subscribe(
        res => {
          this.posted = true;
          this.alertService.success({
  ...
  rejectItems(): any {
    ...
    if (itemsRejected.length > 0) {
      this.others.postRejectReversePrincipal(itemsRejected).subscribe(
        res => {
          this.posted = true;
          this.alertService.success({
